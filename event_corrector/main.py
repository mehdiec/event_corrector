...
class CellClassificationWidget(QWidget):
    def __init__(self, viewer: napari.Viewer, global_path, tif_files, file_id):
        super().__init__()
        self.viewer = viewer
        ...
        self.name = self.tif_files[self.file_id].stem

        self.init_single_seq()

        self.setup()

    def init_single_seq(self):
        # super().__init__()
        print(self.file_id)
        ...
        image_data = skimage.io.imread(self.global_path / f"{self.name}.tif")
        # mask = skimage.io.imread(path / f"masks/mask_{name}_raw.tif")
        save_path_dying_correction = (
            self.global_path / f"masks/apop_detection_{self.name}_correction.tif"
        )
        save_path_dividing_correction = (
            self.global_path / f"masks/division_detection_{self.name}_correction.tif"
        )

        original_dying_path = self.global_path / f"masks/apop_detection_{self.name}.tif"
        original_dividing_path = (
            self.global_path / f"masks/division_detection_{self.name}.tif"
        )
        ...
        labels = skimage.io.imread(self.global_path / f"masks/mask_{self.name}.tif")
        labels = labels.astype(np.int16)
        # mask = mask  # [:, y_start:y_end, x_start:x_end]

        dividing = dividing > 0  # Assuming last but one channel are dividing cells
        dying = dying > 0  # Assuming last channel are dying cells

        # Adding the image and labels layers to the viewer
        self.viewer.add_image(image_data, name="raw_image")
        ...
        if os.path.exists(self.global_path / f"masks/apop_pred_{self.name}.tif"):
            dying_proba = skimage.io.imread(
                self.global_path / f"masks/apop_pred_{self.name}.tif"
            )
            dividing_proba = skimage.io.imread(
                self.global_path / f"masks/division_pred_{self.name}.tif"
            )
    ...
    def display_cell_outlines(self, labels):
        ...
        self.outline_layer = self.viewer.add_labels(
            np.array(outlines), name="outline ", visible=False
        )
    ...
    def setup(self):
        ...
        btn_next_sequence = QPushButton("Next Sequence")
        ...
        layout.addWidget(btn_next_sequence)
        ...
        btn_next_sequence.clicked.connect(self.get_next_sequence)
    ...
    def save_annotations(self):
        ...
        save_path_mask = self.global_path / f"masks/mask_{self.name}_correction.tif"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Process TIFF files to generate masks."
    )
    parser.add_argument("global_path", type=str, help="Folder containing TIFF files")

    parser.add_argument(
        "--file_id", type=int, default=0, help="Device to run the model on"
    )
    ...
    # i want to check all the files that end with tif sorted and in a list

    # List and sort all files ending with '.tif'
    tif_files = sorted(global_path.glob("*.tif"))

    # Initialize our widget and pass the viewer
    cell_widget = CellClassificationWidget(viewer, global_path, tif_files, file_id)
